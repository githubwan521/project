...
-- Records of play
-- ----------------------------
INSERT INTO `play` VALUES ('1', '9', '11', '战狼2',
                           '被开除军籍的冷锋本是因找寻龙小云来到非洲，但是却突然被卷入一场非洲国家的叛乱。因为国家之间政治立场的关系，中国军队无法在非洲实行武装行动撤离华侨。而作为退伍老兵的冷锋无法忘记曾经为军人的使命，本来可以安全撤离的他毅然决然地回到了沦陷区，孤身一人带领身陷屠杀中的同胞和难民，展开生死逃亡。随着斗争的持续，体内的狼性逐渐复苏，最终闯入战乱区域，为同胞而战斗。',
                           NULL, '120', '35.00', '0');
INSERT INTO `play` VALUES ('2', '8', '11', '侠盗联盟',
                           ...
                           NULL, '108', '30.00', '0');
INSERT INTO `play` VALUES ('3', '9', '12', '敦刻尔克',
...
CREATE TABLE `user` (
  ...
  COMMENT '用户类型：0为普通用户，1是管理员',
  `head_path` VARCHAR(500)         DEFAULT NULL
  COMMENT '头像路径',
  PRIMARY KEY (`emp_no`),
  CONSTRAINT `user_ibfk_1` FOREIGN KEY (`emp_no`) REFERENCES `employee` (`emp_no`)
    ON DELETE CASCADE
    ON UPDATE CASCADE
)
  ENGINE = InnoDB
